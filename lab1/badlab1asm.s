...
AsmFunction:
	...
	mov		al, byte [bp-6] ; actually put c into al
	cbw ; sign-extend c
	mov		bx, ax ; put c into bx now

	mov		al, byte [bp-8] ; actually put b into ax
	cbw	; sign-extend al to ax.

	add		ax, bx ; b = b+c
	imul		word [bp-10] ; b = b*a

	mov		bx, word [bp-4] ; Put d into bx
	sub		bx, word [bp-2] ; d = d-e
	idiv		bx	; ax = dx:ax / bx
