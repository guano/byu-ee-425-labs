...
AsmFunction:
	...
	;What I thought it should be based on documentation
		;	mov		al, byte [bp+6]	; Put b into ax
	...
	;What I thought it should be based on documentation
		;	mov	bl, byte[bp+8]	; convert c to a word?
	;Testing that a is actually in bp-10
		;mov cx, word [bp-10] ;just to check that this is actually a.

	add		ax, bx ; b = b+c
	imul		word [bp-10] ; b = b*a
	; now our value is DX:AX = b*a
	; This is a 32-bit value. And we have to treat it as such.
	; which the div operation already does! YES!
	;
	;;;;;;;;;;;;;;;;;;;;;
	; THIS IS A HACK.
	; For the 3rd call, we have Ax = -103 and what is stored in bp-10 is 17.
	; Multiplying those we get Ax = -1751 as expected,
	; but DX changes to 16 for some reason >:(
	; It REALLY should detect that AX is negative and go negative itself (0xFFFF)
