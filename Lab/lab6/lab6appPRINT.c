...
void ATask(void)                /* processes data in messages */
{
	...
	printString("a task while 1\n");
    while (1)
    {
        tmp = (struct msg *) YKQPend(MsgQPtr); /* get next msg */
                
		printString("retrived messege from queue ");
		printInt((int) MsgQPtr);
		printString(" tick = ");
		printInt(tmp->tick);
		printString(" data = ");
		printInt(tmp->data);
...
void STask(void)                /* tracks statistics */
{
	...
	printString("delaying myself 1 tick\n");
    YKDelayTask(1);
    YKIdleCount = 0;
	printString("delaying myself 5 more ticks\n");
    YKDelayTask(5);
    max = YKIdleCount / 25;
    YKIdleCount = 0;

	printString("newtask b\n");
    YKNewTask(BTask, (void *) &BTaskStk[TASK_STACK_SIZE], 10);
	printString("newtask a\n");
    YKNewTask(ATask, (void *) &ATaskStk[TASK_STACK_SIZE], 20);
    
	printString("while 1 STask\n");
    while (1)
    {
        YKDelayTask(20);
